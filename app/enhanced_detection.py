...
class EnhancedAnomalyDetector:
    ...
    def process_anomaly(self, frame, anomaly_type, cae_score, convlstm_score, 
        ...
        # 3. Enviar notificação (assíncrona)
    ...
    def run_detection(self, video_source=0):
        ...
        # Mensagem apropriada baseada no tipo de fonte
        if isinstance(video_source, int):
            print(f"🎥 Iniciando detecção na câmera: {video_source}")
        else:
            print(f"🎥 Iniciando detecção em: {video_source}")
        
        # Carregar modelos e limiares
        self.load_models()
        self.load_thresholds()
        
        # Abrir vídeo ou câmera
        cap = cv2.VideoCapture(video_source)
        if not cap.isOpened():
            print(f"❌ Erro ao abrir fonte de vídeo: {video_source}")
            return
...
def main():
    if len(sys.argv) == 1:
        # Sem argumentos = usar câmera padrão (0)
        print("📹 Modo câmera (padrão)")
        video_source = 0
    elif sys.argv[1].lower() in ['camera', 'cam', 'webcam']:
        # Argumento "camera" = usar câmera padrão
        video_source = 0
    elif sys.argv[1].isdigit():
        # Argumento numérico = ID da câmera
        video_source = int(sys.argv[1])
    else:
        # Argumento é caminho de arquivo
