...
IMG_CHANNELS = 2
SEQUENCE_LENGTH = 10  # ✅ CORRIGIDO: 9 frames de entrada + 1 para previsão (modelo treinado com 9)
    ...
    def run_detection(self, video_source):
        ...
        Args:
            video_source: Pode ser:
                - Caminho para arquivo de vídeo (str): 'video.mp4'
                - ID da câmera (int): 0 para webcam padrão, 1 para segunda câmera, etc
                - URL de stream (str): 'rtsp://...' ou 'http://...'
        """
        # Detectar tipo de fonte
        if isinstance(video_source, str):
            # Verificar se é um arquivo local
            if not video_source.startswith(('rtsp://', 'http://', 'https://')):
                if not os.path.exists(video_source):
                    print(f"❌ Vídeo não encontrado: {video_source}")
                    return
                print(f"🎥 Iniciando detecção em arquivo: {video_source}")
            else:
                print(f"🌐 Iniciando detecção em stream: {video_source}")
        else:
            print(f"📹 Iniciando detecção em câmera ID: {video_source}")
        ...
        # Para câmeras ao vivo ou valores inválidos, usar FPS padrão
        is_live_camera = isinstance(video_source, int)
        
        if original_fps == 0 or original_fps > 120:
            if is_live_camera:
                original_fps = 30  # FPS padrão para webcam
                delay_ms = 1  # Delay mínimo para câmera ao vivo (mais responsivo)
                print(f"📹 Câmera ao vivo detectada | FPS assumido: {original_fps}")
            else:
                original_fps = 30  # Fallback para vídeos
...
def main():
    """
    Uso:
        python enhanced_detection_camera.py <fonte>
    
    Exemplos:
        python enhanced_detection_camera.py video.mp4           # Arquivo de vídeo
        python enhanced_detection_camera.py 0                    # Webcam padrão
        python enhanced_detection_camera.py 1                    # Segunda câmera
        python enhanced_detection_camera.py rtsp://cam_url       # Stream IP
    """
    if len(sys.argv) != 2:
        print("=" * 60)
        print("USO: python enhanced_detection_camera.py <fonte>")
        print("=" * 60)
        print("\n📹 Fontes suportadas:")
        print("  • Arquivo de vídeo: video.mp4")
        print("  • Webcam padrão: 0")
        print("  • Segunda câmera: 1")
        print("  • Stream RTSP: rtsp://usuario:senha@ip:porta/stream")
        print("  • Stream HTTP: http://ip:porta/video")
        print("=" * 60)
        sys.exit(1)
    
    video_source = sys.argv[1]
    
    # Converter para int se for um número (ID de câmera)
    try:
        video_source = int(video_source)
    except ValueError:
        pass  # Manter como string (caminho de arquivo ou URL)
